    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult PatientRecord(Guid id)
        {
            ...
            using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
            {
                ...
                var detailRecord = workScope.PatientRecords.Include(s =>s.Doctor)
                    .FirstOrDefault(x => x.Id == id);
                var record = workScope.Records.Include(x => x.Doctor).FirstOrDefault(x => x.Id == detailRecord.RecordId);
                ViewBag.Record = record; 

                var detailRecords = workScope.DetailRecords.Query(x => x.RecordId == detailRecord.RecordId && !x.IsMainRecord).OrderByDescending(x => x.Process).ToList();

                var detailRecordMain = workScope.DetailRecords.FirstOrDefault(x => x.RecordId == detailRecord.RecordId && x.IsMainRecord);

                ViewBag.DetailRecordMain = detailRecordMain;
