                ...
                if (isEdit) //update
                {
                    using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
                    {
                        ...
                        if (elm != null) //update
                        {
                            ...
                            {
                                var patientDoctor = workScope
                                   .PatientDoctors
                                   .FirstOrDefault(x => x.PatientId == input.PatientId && x.DoctorId == input.DoctorId.Value);

                                if (patientDoctor == null)
                                {
                                    workScope.PatientDoctors.Add(new PatientDoctor
                                    {
                                        DoctorId = input.DoctorId.Value,
